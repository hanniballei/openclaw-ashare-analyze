...
- `THEME_ANALYZE` 额外支持 `板块 / 领域 / 行业 / 方向` 类问法
- 主题优质股推荐能力已经可用：`THEME_ANALYZE` 可返回 `top N` 排名和评分理由
...
python scripts/analyze_theme.py --query "推荐光纤板块 5 只优质股票" --top 5 --compact
python scripts/analyze_theme.py --query "推荐航天航空领域的股票" --top 5 --compact
```
