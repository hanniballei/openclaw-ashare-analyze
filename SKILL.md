...
1. Classify the request into one data scenario or orchestration mode:
   ...
   - `THEME_ANALYZE`: theme / concept / sector questions such as "怎么看光纤板块"、"推荐航天航空领域的股票" or "卫星通信有哪些头部企业"
...
2. Normalize the symbol or theme before running any scenario script.
   ...
   - If the user asks for theme-level stock recommendations, treat `top` as the number of ranked names to return.
...
4. Run the matching script or orchestration through the skill entrypoint and read the JSON.
   - Use the directory containing this `SKILL.md` as `<skill_root>`.
   - Always call `python3 <skill_root>/run.py ...`; do not call `scripts/*.py` directly.
   - `python3 <skill_root>/run.py --task stock_analyze --query "分析通富微电"`
   - `python3 <skill_root>/run.py --task etf_analyze --symbol 159206`
   - `python3 <skill_root>/run.py --task market_overview --query "今天 A 股走势如何"`
   - `python3 <skill_root>/run.py --task trading_strategy --query "29.3 建仓 200 股的通富微电今天怎么操作"`
   - `python3 <skill_root>/run.py --task us_stock --query "看看纳斯达克和 NVDA"`
   - `python3 <skill_root>/run.py --task stock_picker --query "从 159625 里挑几只强势股" --top 10`
   - `python3 <skill_root>/run.py --task theme_analyze --query "怎么看光纤板块" --top 5`
   - For `EVENT_IMPACT`:
...
- For stock / ETF / strategy scenarios, combine `daily`, `4h`, `1h`, and `5min`: `daily` sets the broader structure, `4h` and `1h` confirm swing rhythm, and `5min` checks short-term confirmation or divergence.
- For `US_STOCK`, prefer `daily` only: feed the last 30 daily bars plus the daily indicator snapshot and `daily_summary`; do not force intraday bars unless the user explicitly asks for them.
...
- Use `scripts/analyze_theme.py` for 板块 / 概念 / 主题 / 赛道 / 领域 / 行业 / 方向 questions.
- The public entrypoint is `python3 <skill_root>/run.py --task theme_analyze ...`; the legacy `scripts/analyze_theme.py` wrapper now delegates to that entrypoint for compatibility.
- The script returns a theme-level payload, top-N ranked stocks with score reasons, and representative A-share snapshots with multi-timeframe data.
- To control payload size, only the first 2 representative stocks carry the heavier `daily` / `1h` / `5min` raw bars and matching indicator snapshots; use `ranking` for the broader top-N view.
- When the user asks for "推荐这个板块的优质股票" or "挑 3-5 只最强的票", rely on the `ranking` array first.
...
- The public entrypoint is `python3 <skill_root>/run.py --task us_stock ...`.
- Prefer 30 trading days of `daily` bars plus `daily_summary` for US names and indices; this keeps the payload lighter and more stable than forcing intraday bars.
- `daily_summary` is the main compact layer for US output: use returns, position in the 30-day range, distance to MA20, and volume vs 20-day average to support the explanation.
...
- Standardized entrypoint failures from `run.py` return structured JSON with `error.code`, `error.message`, and `error.task`.
